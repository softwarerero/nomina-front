...
var defaults = {
  ...
  deselectTemplate   : '<span class=deselectstartover><button class="btn btn-large btn-primary" type="button">Reniciar Filtros</button></span>',
...
function createFacetUI() {
  ...
  var deselect = $(settings.deselectTemplate).click(function(event){
    ...
    _.each(settings.facets, function(facettitle, facet) {
    	//$("." + facet).val("-");
    	//$("." + facet).val("-").prop('selected', true);
    	//alert("." + facet + ' option');
    	//$("." + facet + ' option').removeProp('selected');
    });
